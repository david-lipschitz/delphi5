...
uses VBMain, BRModal, PickDate, VBBarCde, VBUtils;
...
begin
  ...
  Product.Post;
  MainForm.Database.Commit;
  Line1:='Rcpt '+ZeroFill(WIPRcptRefNo.Value,6)+
    ' Lot No. '+inttostr(WIPRcptLotNo1.Value)+'/'+
    inttostr(WIPRcptLotNo2.Value);
  Line2:=ShowProdLabelDescr.Text;
  if WIPRcptShift.Value='W' then
    Line2:='eomWIP '+Line2
  else
    Line2:='WIP '+Line2;
  if ShowProdMeasUnit.Text='KGS' then
    ...
    Line3:='Units: '+
      ...
      ShowProdMeasUnit.Text;
  Line4:='Date Manu: '+formatdatetime('dd/mm/yyyy',WIPRcptDateManu.Value);
  Line5:='';
  BarCode:='13'+ZeroFill(WIPRcptRefNo.Value,6);
...
begin
  ...
  WipRcptFactoryNo.Value:=2; {default to factory 2(Weaving) DL100697}
  WipRcptQlty.Value:=''; //added DL070701
  if DefaultsSet then
  begin
...
begin
  ...
  begin
    ...
    if (ShowProdCategory.Text='PAK') and (WIPRcpt.State=dsInsert) then
    begin
...
{      (WIPRcptRcptInitials.Value='') then}
    ...
    LotNo1:=0; {WIPRcptLotNo1.AsInteger;}
    LotNo2:=0; {WIPRcptLotNo2.AsInteger;}
...
procedure TWIPRcptForm.PrintSpdBtnClick(Sender: TObject);
begin
  Print;
end;
